...
----------------------------------------------------------------------------*/
bool
BintxtSinkImpl::open(std::string const &filename)
{
    if (file_) {
        return false;
    }

    file_ = fopen(filename.c_str(), "a+b");
    return file_ != NULL;
}
...
----------------------------------------------------------------------------*/
bool
BintxtSink::open(std::string const &opts)
{
    std::string filename(opts.length() > 0 ? opts : "filedb.bin");
